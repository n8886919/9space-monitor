...
9Space Snapshot app 拉取截圖，並接收既有的去敏 telemetry push。
...
站點資料由各站 Snapshot app 隨 telemetry 自動註冊。Snapshot app options 中：
...
- `hub_ip` 只填中央 Hub 的 Tailscale IPv4 或完整 `*.ts.net` hostname。Snapshot
  app 固定使用 HTTP、port `8765` 與 `/api/v1/telemetry`，不接受 scheme、port 或 path。
