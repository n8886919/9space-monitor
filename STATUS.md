...
- Source/current test deployments: Snapshot add-on `0.3.11`; Hub add-on `0.3.2`; `nvr_monitor` integration `0.2.8`。
...
- Root tests `97/97`、`155` subtests PASS；最新相關 Snapshot／Hub suites `62/62` PASS；compileall、shell syntax 與 `git diff --check` PASS。本機 Docker build 因無 daemon permission FAIL，兩站 Supervisor managed update/build PASS。
...
- `nvr_monitor` `0.2.8` 已原子部署至 daan-forest 與 chengde；兩站 `ha core check`、Core recovery、版本 gate PASS，restart 後去敏 `nvr_monitor` error count `0`。
...
1. 修復 chengde Camera 09 的 local NVR／RTSP 問題，恢復第 14 路 snapshot。
2. 驗證 Hub camera/current-state entities、Recorder history 與正式 Dashboard；不以 debug Web UI 作正式 UI 驗收。
3. 製作非工程人員只看截圖的正式 Dashboard，以及工程用站點表格／current statistics view。

## Blockers

- chengde Camera 09 local RTSP／recording query 仍失敗；正式 Dashboard 與 Recorder UI 尚未驗收。
