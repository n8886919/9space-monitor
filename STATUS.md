...
- Functional release commit: `9a6bdb3`.
- Local/deployed versions: add-on `0.3.8`; integration `0.2.6`.
...
- Release commit `9a6bdb3` 已 push 至 task branch，並以 fast-forward 發布至 `origin/main`。
...
1. 由使用者確認在線率與斷線次數已從零／首次 observation 重新累積；確認後清除本次 integration deployment transaction。
...
技術部署沒有 blocker；新 integration RAM history 已重新開始，尚待使用者確認 UI 顯示。
...
- Integration `0.2.6` deployment transaction 暫時保留，待使用者確認 UI runtime 後清除。
