...
- Branch: `main`.
- Functional release commits: `023170b` 與 `8229d86`，已 fast-forward push 至 `origin/main`；未 force push、未改寫歷史。
- Source/current central test deployment: Snapshot add-on `0.3.9`; Hub add-on `0.2.0`; `nvr_monitor` integration `0.2.7`。
...
- Hub 不再保存或要求 per-site options。Snapshot add-on 隨 telemetry 在 RAM 自動註冊站點；channel count、concurrency、timeout 沿用站點既有設定，只新增 Tailscale Snapshot API origin 與 Hub refresh interval。
...
- Root tests `93/93` PASS；本次相關 Snapshot／Hub suites `118/118` PASS；compileall、shell syntax 與 `git diff --check` PASS。完整 203-item suite 在既有 Snapshot lifecycle tests 於本 sandbox TIMEOUT；本機 Docker build 因無 daemon permission FAIL，Supervisor managed update/build PASS。

## Deployed

- 中央 Home Assistant 已安裝 `custom_components/nine_space_monitor_hub` source；本次 `ha core check` PASS，未編輯 `.storage`。
- 中央 HA 已由既有 managed repository 更新 Hub `0.2.0` 與 Snapshot add-on `0.3.9`；兩者 state `started`、version gate PASS。Hub tailnet `8765/healthz` 與 Snapshot `8222/healthz` PASS。
...
1. 各站 Snapshot add-on 更新至 `0.3.9`，在 UI 設定 Hub telemetry endpoint、該站可由 Hub 連線的 Tailscale Snapshot API origin，以及 Hub refresh interval；既有 site/display/channel/concurrency/timeout 繼續沿用。
...
- 每站 private Snapshot options 與 component config entry 是使用者負責的 HA UI gate；目前無 live Hub entity／snapshot 驗證。
