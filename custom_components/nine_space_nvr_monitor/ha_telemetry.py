...
# intentionally filtered before a Hub producer is built.
...
class HATelemetryProducer:
    """Bounded RAM producer; sampling never awaits Hub network I/O."""
    ...
    async def _run(self) -> None:
        while not self._stopping:
            ...
            except Exception:  # Hub failure is intentionally isolated from HA lifecycle.
                self.dropped_events += len(events)
            finally:
