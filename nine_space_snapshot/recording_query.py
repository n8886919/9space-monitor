...
``custom_components/nine_space_nvr_monitor/recording.py`` (``DahuaRecordingClient``).
...
- Returns only the small, already-redacted fields the app API exposes
...
# assumption (custom_components/nine_space_nvr_monitor/recording.py LOCAL_TZ) so the
...
class NvrHttpConfig:
    """Dahua NVR CGI connection settings (app scope only)."""
