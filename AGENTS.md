...
- App 是唯一可直接連接 Dahua NVR、保存 NVR host／ports／username／password、建立 RTSP URL、呼叫 Dahua CGI、執行 ffmpeg snapshot 的元件。
- Local `nine_space_nvr_monitor` integration 只透過 local app API 取得 NVR／錄影狀態；不得取得 snapshot、建立 Snapshot camera entity 或重做 Home Assistant Ping integration。中央 `nine_space_hub` component 只可經 Hub API 建立跨站 snapshot camera 與 current-state entities。
...
- App／integration producer 只使用 24 小時 RAM ring 與 bounded memory queue，不把 telemetry history 寫入磁碟。
- Hub app 不保存 telemetry／snapshot attempt history 或使用 SQLite；current state 只留 RAM，歷史由中央 Home Assistant Recorder 保存。Hub snapshot store 只能 bounded、atomic replace 地保存每個 site/channel 唯一一張最後成功 JPEG，且不得建立 history 或把 JPEG 寫入 telemetry、log、fixture、Git。
- Hub 只能經站點 app snapshot API 取圖，不能直接連 NVR。每站 channel count 由 options mapping 明定，不可假設固定數量或命名。
...
- 不改 repository 名稱、integration domain `nine_space_nvr_monitor`、app slug `9space_snapshot`、container port `8000` 或既有 host port `8122`。
